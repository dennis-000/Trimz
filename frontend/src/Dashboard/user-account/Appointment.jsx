...
const Appointments = ({ appointments, refreshAppointments }) => {
  ...
  const handleEdit = (appointment) => {
    ...
    setEditFormData({
      ...
      // Add any other editable fields here if needed
    });
  };
  ...
  const handleUpdateAppointment = async () => {
    try {
      ...
      let formattedTime = editFormData.date.split('T')[0] + 'T' + editFormData.startTime + ':00.000Z';
      ...
      const res = await fetch(`${BASE_URL}appointments/${editingAppointment._id}`, {
        method: "PATCH",
        headers: {
            ...
            <tbody>
              {appointments.map(item => (
                <tr key={item._id} className="bg-white border-b hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">{(new Date(item.startTime)).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}</td>
          ...
          {appointments.map((item) => (
            <div key={item._id} className="bg-white p-4 rounded-lg shadow space-y-3">
              <div className="flex items-center space-x-3">
                <img
                  src={item.provider.profilePicture.url}
      ...
      {editingAppointment && (
        <div className="fixed inset-0 flex items-center justify-center z-50">
          <div
            ...
            onClick={closeModal}
          ></div>
          <div className="bg-white p-6 rounded-lg z-10 w-11/12 md:w-1/2">
              ...
              <input
                ...
                value={
                  editFormData.startTime
                    ? // If the startTime contains a "T", assume it's an ISO string and convert it; otherwise, use it directly.
